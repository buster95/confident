...
use Confident\DataBase as DB;
use Confident\Utilities\JsonHelper;

define('DESC', 'DESC', true);
define('ASC', 'ASC', true);
// require_once ('DB.php');
// require_once ($_SERVER['DOCUMENT_ROOT'].'/php/confident/DB.php');
...
class Table
{
    ...
    public function COLUMN_TYPE($columna = '')
    {
        ...
        while ($fila = $columnas->fetch_array(MYSQLI_ASSOC)) {
            ...
            if ($this->search_array($number, $tipo)) {
                ...
                return "BOOLEAN";
            } elseif ($this->search_array($binario, $tipo)) {
                return "BINARY";
            }
        }
     ...
     * @throw Error si el nombre no se acepta
     */
    ...
    public function whereOr($parametro, $operador, $valor = '', $clean_special = false)
    {
        ...
        } elseif ($this->COLUMN_TYPE($parametro) === 'STRING' && (is_string($valor) && $valor !== 0)) {
            $this->where .= " LIKE '" . $valor . "'";
        } elseif ($this->COLUMN_TYPE($parametro)==='BOOLEAN') {
            $this->where .= "=".$valor;
        } else {
            throw new Exception("COLUMNA TIPO DE DATO DIFERENTE AL VALOR", 1);
        }
    ...
    {
        $resultado = $this->mydb->consultar($this->getSQL());
